...
def request_dynamic(request):
	...
	if request.method == "POST":
		...
		for i in range(51):
			...
			y_longitude = float(longitude) + longitude_CONSTANT*(math.sin(3.14*0.02*i))
			ROADS_API_URL = ROADS_API_URL+str(x_latitude)+','+str(y_longitude)+'|'
		ROADS_API_URL = ROADS_API_URL[:(len(ROADS_API_URL)-1)]
		ROADS_API_URL = ROADS_API_URL + '&key=AIzaSyCOhrN9Tbo_a1sxm9Kcy2zxb4C7b51XB1k'
		print(ROADS_API_URL)
		...
		w = TrafficModel.objects.values('place_id').annotate(total_avg_speed=Avg('avg_speed')).order_by('total_avg_speed')
		print(w)
		traffic_places_list = []
		for member in w :
...
def getplace_name_from_id(placeid):
	r = requests.get("https://maps.googleapis.com/maps/api/place/details/json?"\
					 "placeid="+placeid+"&key=AIzaSyB5OBkGEowp58z-e8NoCmWeHlV2osjZgqc")
	print(r.text)
...
def get_place_id(lat, lng):
	...
	r = requests.get('https://roads.googleapis.com/v1/nearestRoads?' \
					 'points=' + points + '&key=AIzaSyCOhrN9Tbo_a1sxm9Kcy2zxb4C7b51XB1k')
...
def make_api_requests(s_lat, s_lng, e_lat, e_lng):
	API_URL = 'https://maps.googleapis.com/maps/api/directions/' \
			  'json?origin=' + str(s_lat) + ',' + str(s_lng) + '&destination=' + str(e_lat) + ',' + str(e_lng) + \
...
class Path:
	...
	def calculate_traffic_score(self):
		try:
			self.traffic_score = (self.distance / self.time_actual) - (self.distance / self.time_in_traffic)
